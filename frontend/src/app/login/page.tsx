    ...
    return (
        <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-hidden">
            ...
            <div className="w-full max-w-md mb-4 relative z-10">
                <Link href="/" className="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-300 text-xs font-semibold transition-colors">
                    ← Back to Home
                </Link>
            </div>
            ...
            >
                <div className="text-center mb-6 sm:mb-8">
                    <Link href="/" className="w-12 h-12 sm:w-16 sm:h-16 bg-primary/20 rounded-2xl flex items-center justify-center mx-auto mb-4 sm:mb-6 border border-primary/20 hover:scale-105 transition-transform">
                        <Logo className="w-7 h-7 sm:w-10 sm:h-10" />
                    </Link>
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mb-2">Grantify Access</h1>
                    <p className="text-slate-500 text-xs sm:text-sm">Empowering Nigerian Growth through Strategic Grants</p>
                </div>

                <div className="glass-card rounded-[1.75rem] sm:rounded-[2.5rem] p-6 sm:p-10 border-white/10 shadow-2xl relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary via-secondary to-primary/50" />

                    {/* Tab Toggle */}
                    <div className="flex mb-6 sm:mb-8 bg-white/5 p-1 rounded-xl">
                        <button
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                        ...
                        {isSignUp && (
                            <div
                                className="flex items-start gap-3 p-3 sm:p-4 bg-primary/5 rounded-2xl border border-primary/10 group cursor-pointer"
                                onClick={() => setAgreedToEligibility(!agreedToEligibility)}
                            >
                                ...
                                <p className="text-[10px] sm:text-[11px] font-semibold text-slate-400 leading-relaxed select-none">
                        ...
                        <div className="flex flex-col gap-3 pt-1 sm:pt-2">
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full py-3.5 sm:py-4 bg-primary text-background rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-primary/20 disabled:opacity-50"
                            >
                                {loading
                                    ? "Syncing Gateway..."
                ...
                {/* Trust Footer */}
                <div className="mt-5 sm:mt-8 flex flex-wrap items-center justify-center gap-4 sm:justify-between sm:px-6">
